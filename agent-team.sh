...
SKILLS_DIR="$AGENT_TEAM_HOME/skills"
...
cmd_provision() {
  ...
  # --prune: 팀팩에 없는 에이전트를 폴더에서 제거(중복·잔재 정리). 백업 후 삭제.
  local pruned=0
  if [ "$prune" -eq 1 ]; then
    local wl="$dest/.wanted.tmp"; : > "$wl"
    for name in "${wanted[@]}"; do echo "$name" >> "$wl"; done
    local bdir="$proj/.claude/agents_pruned_backup"
    while IFS= read -r df; do
      [ -n "$df" ] || continue
      local dn; dn=$(basename "$df" .md)
      if ! grep -qxF "$dn" "$wl"; then
        mkdir -p "$bdir"; mv "$df" "$bdir/"
        pruned=$((pruned+1))
        log "   ${C_YELLOW}정리(제거)${C_RESET}: $dn  ${C_DIM}→ agents_pruned_backup/${C_RESET}"
      fi
    done < <(find "$dest" -maxdepth 1 -type f -name '*.md')
    rm -f "$wl"
  fi
...
# ══════════════════════════════════════════════════════════════════════
# skill-import — GitHub 스킬 모음 repo(또는 로컬 폴더)에서 SKILL.md 일괄 수집
#   → agent-team/skills/ 에 저장(git 공유) + ~/.claude/skills/ (또는 프로젝트)에 설치
# ══════════════════════════════════════════════════════════════════════
cmd_skill_import() {
  local src="" install_to="global" proj="" no_install=0
  while [ $# -gt 0 ]; do
    case "$1" in
      --global) install_to="global" ;;
      --project) shift; install_to="project"; proj="${1:-}" ;;
      --no-install) no_install=1 ;;
      -*) die "알 수 없는 옵션: $1" ;;
      *) [ -z "$src" ] && src="$1" || die "인자 과다: $1" ;;
    esac
    shift
  done
  [ -n "$src" ] || die "사용법: skill-import <REPO_URL|디렉터리> [--global | --project DIR] [--no-install]"

  # 1) 소스 확보 (URL 이면 임시 clone, 로컬이면 그대로)
  local srcdir cleanup=""
  if [ -d "$src" ]; then
    srcdir="$(abspath "$src")"
  else
    command -v git >/dev/null 2>&1 || die "git 필요 (repo clone 용)"
    srcdir="$(mktemp -d 2>/dev/null || echo "/tmp/skimport.$$")"
    cleanup="$srcdir"
    info "스킬 repo clone 중… ${C_DIM}$src${C_RESET}"
    git clone --depth 1 "$src" "$srcdir" >/dev/null 2>&1 || { rm -rf "$cleanup"; die "clone 실패: $src"; }
  fi

  # 2) SKILL.md 전부 탐색
  mkdir -p "$SKILLS_DIR"
  local imported=0 dest_install=""
  if [ "$no_install" -ne 1 ]; then
    if [ "$install_to" = "project" ]; then
      [ -n "$proj" ] || { [ -n "$cleanup" ] && rm -rf "$cleanup"; die "--project 뒤에 프로젝트 경로 필요"; }
      dest_install="$(abspath "$proj")/.claude/skills"
    else
      dest_install="$HOME/.claude/skills"
    fi
    mkdir -p "$dest_install"
  fi

  info "SKILL.md 탐색: ${C_DIM}$srcdir${C_RESET}"
  while IFS= read -r sf; do
    [ -n "$sf" ] || continue
    local sdir; sdir="$(dirname "$sf")"
    # 스킬 이름: 프론트매터 name → 없으면 폴더명
    local nm; nm=$(front_field "$sf" "name"); [ -n "$nm" ] || nm="$(basename "$sdir")"
    # 안전한 폴더명으로 정규화
    nm=$(printf '%s' "$nm" | tr ' /' '__' | tr -cd 'A-Za-z0-9_.-')
    [ -n "$nm" ] || continue
    local desc; desc=$(front_field "$sf" "description" | cut -c1-70)
    # 공유 저장소에 복사 (스킬 폴더 통째로)
    rm -rf "$SKILLS_DIR/$nm"; mkdir -p "$SKILLS_DIR/$nm"
    cp -R "$sdir/." "$SKILLS_DIR/$nm/" 2>/dev/null || cp "$sf" "$SKILLS_DIR/$nm/SKILL.md"
    # 설치 대상에도 복사
    if [ "$no_install" -ne 1 ]; then
      rm -rf "$dest_install/$nm"; mkdir -p "$dest_install/$nm"
      cp -R "$SKILLS_DIR/$nm/." "$dest_install/$nm/"
    fi
    imported=$((imported+1))
    printf "  ${C_GREEN}▪${C_RESET} %-28s ${C_DIM}%s${C_RESET}\n" "$nm" "$desc"
  done < <(find "$srcdir" -type f -name 'SKILL.md' 2>/dev/null | sort)

  [ -n "$cleanup" ] && rm -rf "$cleanup"

  log ""
  if [ "$imported" -eq 0 ]; then
    warn "SKILL.md 를 못 찾았습니다. repo 구조를 확인하세요 (스킬은 각 폴더의 SKILL.md)."
    return 0
  fi
  ok "스킬 ${C_BOLD}${imported}개${C_RESET} 수집 → 공유저장소: $SKILLS_DIR"
  [ "$no_install" -ne 1 ] && ok "설치 완료 → $dest_install  ${C_DIM}(claude 새 세션에서 /skills 로 확인)${C_RESET}"
  warn "⚠️ 스킬은 제3자 지시문·스크립트를 포함할 수 있습니다. 사용 전 각 SKILL.md 를 검토하세요."
  log "   ${C_DIM}공유: cd $AGENT_TEAM_HOME && git add skills && git commit -m 'skills' && git push${C_RESET}"
  log "   ${C_DIM}다른 맥: git pull 후  agent-team skill-install  로 설치${C_RESET}"
}

# skill-install — 공유저장소(agent-team/skills)의 스킬을 이 맥/프로젝트에 설치
cmd_skill_install() {
  local install_to="global" proj=""
  while [ $# -gt 0 ]; do
    case "$1" in
      --global) install_to="global" ;;
      --project) shift; install_to="project"; proj="${1:-}" ;;
  ...
  [ -d "$SKILLS_DIR" ] || die "공유 스킬 없음: $SKILLS_DIR  (먼저 skill-import 하세요)"
  local dest
  if [ "$install_to" = "project" ]; then
    [ -n "$proj" ] || die "--project 뒤에 경로 필요"
    dest="$(abspath "$proj")/.claude/skills"
  else
  ...
  while IFS= read -r sd; do
    [ -n "$sd" ] || continue
    local nm; nm="$(basename "$sd")"
    rm -rf "$dest/$nm"; mkdir -p "$dest/$nm"; cp -R "$sd/." "$dest/$nm/"
...
_dept_of() {
  ...
  case "$n" in
    ...
    *qa*|*test*|*review*|*critic*|*품질*|*검증*)                       echo "QA·품질" ;;
    ...
    *quant*|*math*|*analyst*|*research*|*news*|*harvest*|*data*|*explore*|*scout*|*수학*|*데이터*|*빅데이터*|*분석*|*리서치*|*탐색*) echo "데이터·리서치" ;;
...
명령:
  ...
  skill-import <REPO_URL|DIR> [--global|--project DIR] [--no-install]
                                             스킬 모음 repo에서 SKILL.md 일괄 수집 →
                                             skills/ 저장(git 공유) + ~/.claude/skills 설치
  skill-install [--global|--project DIR]     공유 skills/ 를 이 맥/프로젝트에 설치
  doctor                                     환경/소스 폴더 점검
